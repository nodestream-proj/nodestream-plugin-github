...
async def test_do_not_retry_bad_status(httpx_mock: HTTPXMock, status_code):
    ...
    httpx_mock.add_response(url=f"{DEFAULT_ENDPOINT}/example?per_page=100", status_code=status_code)
...
async def test_retry_ratelimited(httpx_mock: HTTPXMock):
    ...
    httpx_mock.add_response(url=f"{DEFAULT_ENDPOINT}/example?per_page=100", status_code=200, json=["a","b"], is_reusable=False)
